...
class DenseTrainer(Trainer):
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        logger.info(
            "get_train_dataloader train_batch_size: {}, per_device_train_batch_size: {}".format(
                self.args.train_batch_size, self.args.per_device_train_batch_size
            )
        )
        return DataLoader(
            self.train_dataset,
    ...
    def training_step(self, *args):
        return (
            super(DenseTrainer, self).training_step(*args)
            / self._dist_loss_scale_factor
        )
    ...
    def save_prompt(self, dataset_attributes, output_dir: Optional[str] = None):
        ...
        torch.save(
            general_state_dict, os.path.join(output_dir, "general_state_dict.prompt")
        )
        ...
        for index, state_dict in enumerate(lm_q_prompts):
            torch.save(
                ...
                os.path.join(
                    output_dir,
                    "lm_q_attribute_{}.prompt".format(dataset_attributes[index]),
                ),
            )
        for index, state_dict in enumerate(lm_p_prompts):
            torch.save(
                ...
                os.path.join(
                    output_dir,
                    "lm_p_attribute_{}.prompt".format(dataset_attributes[index]),
                ),
            )
    ...
    def load_prompt(
        ...
        self.model.load_prompt(
            general_prompt_path, attribute_prompt, untie_encoder, dataset_attributes
        )
            ...
            optimizer_grouped_parameters = [
                {
                    "params": [
                        ...
                        if n in decay_parameters and "prefix_encoder" not in n
                    ],
                ...
                },
                {
                    "params": [
                        p
                        for n, p in opt_model.named_parameters()
                        if n not in decay_parameters and "prefix_encoder" not in n
                    ],
                    "weight_decay": 0.0,
                },
                {
                    "params": [
                        p
                        for n, p in opt_model.named_parameters()
                        if "general_prefix_encoder" in n
                    ],
                ...
                },
                {
                    "params": [
                        p
                        for n, p in opt_model.named_parameters()
                        if "attribute_prefix_encoder" in n
                    ],
...
def split_dense_inputs(model_input: dict, chunk_size: int):
    ...
    return [
        {arg_key: c, attr_key: attr_v}
        for c, attr_v in zip(chunked_arg_val, chunked_attrs)
    ]
...
class GCTrainer(DenseTrainer):
    ...
    def training_step(self, model, inputs, num_items_in_batch=None) -> torch.Tensor:
        ...
        queries, passages = {"query": queries, "attrs": attrs}, {
            "passage": passages,
            "attrs": torch.cat([i.repeat(n_passages, 1) for i in attrs], dim=0),
        }
